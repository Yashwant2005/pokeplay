function register_066_candy(bot, deps) {
  ...
  bot.action(/candy_/, check2q, async ctx => {
    ...
    // Fixed: Check if pokemoves exists and has moves_info
    if (((currentLevel + 1) - currentLevel) != 0) {
      
      // Only process moves if pokemoves data exists for this Pokemon
      if (pokemoves[p2.name] && pokemoves[p2.name].moves_info) {
        ...
        const moves2 = moves.moves_info.filter((move) => 
          ...
          move.level_learned_at <= (currentLevel + 1) && 
          dmoves[move.id] && // Check if move exists in dmoves
