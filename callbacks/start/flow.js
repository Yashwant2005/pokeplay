function registerStartFlowCallbacks(bot, deps) {
  ...
  bot.action(/staut_/, async (ctx) => {
    ...
    if (ctx.callbackQuery.data.split('_')[1] == 'back') {
      ...
      await editMessage(
        ...
        '*From Which Region You Wanna Start Your Journey?*',
        { reply_markup: { inline_keyboard: key }, parse_mode: 'markdown' }
      );
      return;
    }

    const regionStarters = starters[ctx.callbackQuery.data.split('_')[1]];
    const key = [[]];
    for (let i = 0; i < regionStarters.length; i++) {
      key[0].push({
        text: regionStarters[i],
        callback_data: 'choose_' + regionStarters[i] + '_' + ctx.callbackQuery.data.split('_')[1] + '_' + ref,
      });
    }
    key.push([{ text: 'Back <-', callback_data: 'staut_back_' + ref }]);
    await editMessage(
      'text',
      ctx,
      ctx.chat.id,
      ctx.callbackQuery.message.message_id,
      '*Which Poke You Wanna Choose As Your Starter*',
      { parse_mode: 'markdown', reply_markup: { inline_keyboard: key } }
    );
  });
  ...
  bot.action(/choose_/, async (ctx) => {
    ...
    const moves2 = moves.moves_info.filter(
      (move) =>
        move.learn_method == 'level-up' &&
        move.level_learned_at < 6 &&
    ...
    if (ref && ref.length > 0) {
      ...
      await sendMessage(
        ctx,
        -1003069884900,
        '#refer\n\n<b>' + he.encode(ctx.from.first_name) + '</b> (<code>' + ctx.from.id + '</code>) has used<b>' + data2.inv.name + '</b> (<code>' + ref + '</code>) refer link.',
        { parse_mode: 'html' }
      );
      await sendMessage(
        ctx,
        ctx.chat.id,
        { parse_mode: 'HTML' },
        'You Have Been Successfully Referred By <b>' + data2.inv.name + '</b>\nReach Level <b>20</b> To Complete Your Refer'
      );
      await sendMessage(
        ctx,
        ref,
        '<b>' + he.encode(ctx.from.first_name) + '</b> has used your refer link.\nYou will get reward when user reaches <b>Level 20</b>',
        { parse_mode: 'HTML' }
      );
    ...
    await editMessage(
      ...
      ctx.chat.id,
      ctx.callbackQuery.message.message_id,
      'You Have Successfully Started Your Journey With *' + c(pokeName) + '*',
      { parse_mode: 'markdown' }
    );
  });
